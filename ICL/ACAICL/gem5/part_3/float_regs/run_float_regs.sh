...
FLOAT_REG_SIZES=("50" "60" "70" "80" "90" "100" "128")
...
run_job() {
  local idx="$1" name="$2" regs="$3" logfile="$4"
  {
    ...
    echo "CMD: $PYTHON_CMD --num-float-phys-regs ${regs} --name ${name}" | tee -a "$logfile"
    
    $PYTHON_CMD \
      --window-size 128,85,43 \
      --num-float-phys-regs "${regs}" \
      --name "./out/${name}" \
      2>&1 | tee -a "$logfile"
...
count=0
for regs in "${FLOAT_REG_SIZES[@]}"; do
    # compose flags
    NAME="FLOATREGS_${regs}"
