"""
Parse gem5 output under `results/out` and plot energy (J) vs number of floating-point physical registers.
...
Usage: run this script from the `part_3/float_regs` directory with the same Python you use to run other scripts.
It will write `results/plots/float_regs_energy.png` and a CSV of values.
"""
...
def extract_reg_count(name: str) -> Optional[int]:
	"""Extract the numeric register count from names like FLOATREGS_128 or 5_FLOATREGS_128.log"""
	m = re.search(r"FLOATREGS_([0-9]+)", name)
	if m:
...
def collect_energy_values(out_dir: str) -> Dict[int, float]:
	...
	for entry in sorted(os.listdir(out_dir)):
		...
		if os.path.isdir(path):
			...
			regs = extract_reg_count(entry)
			if regs is None:
				# maybe directory named FLOATREGS_... (already handled) else skip
				regs = extract_reg_count(entry)
			parsed = parse_metrics_from_text(txt)
			if regs is not None and parsed is not None:
				sim, runtime, sub, gate = parsed
...
def plot_values(values: Dict[int, float], out_dir: str) -> None:
	...
	# save CSV
	csv_path = os.path.join(out_dir, 'float_regs_energy.csv')
	with open(csv_path, 'w', newline='') as cf:
		w = csv.writer(cf)
		w.writerow(['num_float_regs', 'energy_joules'])
		for r, e in zip(regs, energy):
			w.writerow([r, e])

	plt.figure(figsize=(7, 4.5))
	plt.plot(regs, energy, marker='o', linestyle='-')
	plt.xlabel('Number of physical floating-point registers')
	plt.ylabel('Energy (Joules)')
	plt.title('Energy vs number of float physical registers')
	plt.grid(alpha=0.4)

	# mark the minimum energy point with a distinct color and annotation
	try:
		min_val = min(energy)
		min_idx = energy.index(min_val)
		min_reg = regs[min_idx]
		plt.scatter([min_reg], [min_val], color='red', s=90, zorder=5, label='min energy')
		plt.annotate(
			f"min={min_val:.6g} J\nregs={min_reg}",
			xy=(min_reg, min_val),
			xytext=(10, -30),
		...
		# add a legend to show the min marker
		plt.legend(fontsize='small')
	except Exception:
		# In case something unexpected happens (empty list etc.), skip marking
		pass

	png = os.path.join(out_dir, 'float_regs_energy.png')
	pdf = os.path.join(out_dir, 'float_regs_energy.pdf')
	plt.tight_layout()
