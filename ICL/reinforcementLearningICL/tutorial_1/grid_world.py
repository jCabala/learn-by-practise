...
class GridWorld:
    ...
    def get_valid_actions(self, pos: tuple[int, int]) -> list[Action]:
        ...
        for action in Action:
            new_pos = self.get_next_state(pos, action)
            if self._is_passable(new_pos):
                valid_actions.append(action)
        return valid_actions

    def get_next_state(self, pos: tuple[int, int], action: Action) -> tuple[int, int]:
        ...
        if action == Action.UP:
            ...
            next_pos = (row, col - 1)
        if not self._is_passable(next_pos):
            next_pos = pos
        return next_pos
    ...
    def step(self, action: Action) -> tuple[tuple[int, int], float, bool]:
        ...
        # Get next position
        self.agent_pos = self.get_next_state(self.agent_pos, action)
